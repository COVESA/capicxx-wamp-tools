 ...
 * 
 * This is a copy of Franca's AbstractCommandLineTool class (available with Franca 0.11.0).
 * TODO: Remove this after the org.genivi.commonapi.wamp project has been lifted to Franca 0.11.0 or later.
 * 
 * @author Klaus Birken (itemis AG)
 * 
 * @deprecated Use AbstractCommandLineTool class from Franca 0.11.0 instead.
 */
abstract class AbstractCommandLineTool {
	...
	def protected static <T extends AbstractCommandLineTool> void execute(
		...
		execute(new FrancaIDLStandaloneSetup(), toolVersion, concreteClass, args)
	}

	def protected static <T extends AbstractCommandLineTool> void execute(
		FrancaIDLStandaloneSetup standaloneSetup,
		String toolVersion,
		Class<T> concreteClass,
		String[] args
	) {
		injector = standaloneSetup.createInjectorAndDoEMFRegistration();
