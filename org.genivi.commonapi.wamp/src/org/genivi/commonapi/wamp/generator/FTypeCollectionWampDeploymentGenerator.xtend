...
class FTypeCollectionWampDeploymentGenerator {
    ...
    def generateTypeCollectionDeployment(FTypeCollection tc, IFileSystemAccess fileSystemAccess,
        ...
        if(FPreferencesWamp::getInstance.getPreference(PreferenceConstantsWamp::P_GENERATE_CODE_WAMP, "true").equals("true")) {
            fileSystemAccess.generateFile(tc.wampDeploymentHeaderPath, IFileSystemAccess.DEFAULT_OUTPUT,
                tc.generateDeploymentHeader(deploymentAccessor, modelid))
            fileSystemAccess.generateFile(tc.wampDeploymentSourcePath, IFileSystemAccess.DEFAULT_OUTPUT,
                tc.generateDeploymentSource(deploymentAccessor, modelid))
        }
        else {
            // feature: suppress code generation
            fileSystemAccess.generateFile(tc.wampDeploymentHeaderPath, IFileSystemAccess.DEFAULT_OUTPUT, PreferenceConstantsWamp::NO_CODE)
            fileSystemAccess.generateFile(tc.wampDeploymentSourcePath, IFileSystemAccess.DEFAULT_OUTPUT, PreferenceConstantsWamp::NO_CODE)
        }
    }
    ...
    def private generateDeploymentHeader(FTypeCollection _tc,
        ...
        #ifndef «_tc.defineName.toUpperCase»_WAMP_DEPLOYMENT_HPP_
        #define «_tc.defineName.toUpperCase»_WAMP_DEPLOYMENT_HPP_
        ...
        #endif // «_tc.defineName.toUpperCase»_WAMP_DEPLOYMENT_HPP_
    '''
    ...
    def private generateDeploymentSource(FTypeCollection _tc,
                                         ...
                                         IResource _modelid) '''
        «generateCommonApiWampLicenseHeader()»
        #include "«_tc.wampDeploymentHeaderFile»"
    ...
    def protected String generateArrayDeploymentType(FTypeRef _elementType, int _indent) {
        var String deployment = generateIndent(_indent) + "CommonAPI::Wamp::ArrayDeployment<\n"
    ...
    def protected dispatch String generateDeploymentType(FStructType _struct, int _indent) {
        ...
        if (elements.length == 0) {
            ...
            deployment += "CommonAPI::Wamp::StructDeployment<\n"
            for (e : elements) {
                if (e.array) {
    ...
    def protected dispatch String generateDeploymentType(FUnionType _union, int _indent) {
        ...
        if (elements == 0) {
            ...
            deployment += "CommonAPI::Wamp::VariantDeployment<\n"
            for (e : elements) {
                if (e.array) {
                    deployment = deployment + generateArrayDeploymentType(e.type, _indent + 1)
                } else if (e.type.derived != null) {
    ...
    def protected dispatch String generateDeploymentType(FBasicTypeId _type, int _indent) {
        ...
        if (_type == FBasicTypeId.STRING)
            deployment = deployment + "CommonAPI::Wamp::StringDeployment"
        else if (_type == FBasicTypeId.UINT32)
            deployment = deployment + "CommonAPI::Wamp::IntegerDeployment"
    ...
    def protected dispatch String getDeploymentParameter(FBasicTypeId _typeId, EObject _source, PropertyAccessor _accessor) {
        ...
        if (_typeId == FBasicTypeId.STRING) {
            if (_accessor.getWampIsObjectPathHelper(_source)) parameter = "true" else parameter = "false"
        }
        if (_typeId == FBasicTypeId.UINT32 || _typeId == FBasicTypeId.INT32) {
            if (_accessor.getWampIsUnixFDHelper(_source)) parameter = "true" else parameter = "false"
        }        
        return parameter
    }
